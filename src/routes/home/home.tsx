...
function Home(props: {
  ...
  setViewedMsg: (arg0: MessageItem) => void;
}) {
  ...
  const [actionState, setActionState] = useState<MessageActionState | null>(
    null
  );
  ...
  const getFiltered = (): MessageItem[] => {
    const notFlagged = messages
      ? messages.filter((x) => x.status![0] !== "Flagged")
      : [];
    const toShow = showFlagged ? messages : notFlagged;
    let filtered: MessageItem[] = toShow
      ? toShow?.slice(skipped, skipped + 10)
      : [];
  ...
  const modalAction = async (): Promise<void> => {
    ...
    if (actionState === MessageActionState.Deleting) {
      ...
      const newReply = {
        ...
        status:
          actionState === MessageActionState.Flagging ? ["Flagged"] : ["Ok"],
      ...
      try {
        ...
        if (actionState === MessageActionState.Flagging) {
          setAsNotification(
            messageToAction?.username ? messageToAction.username : "",
      ...
      <div className="layout">
        <Row>
          ...
          <Col span={20} lg={12}>
            ...
            <Divider orientation="left" plain={true}>
              {!messages ? (
                ...
                <div>
                  {`Messages total (${
                    getFiltered()?.length ? getFiltered().length : "0"
                  })`}
                </div>
              )}
            </Divider>
                  ...
                  renderItem={(messageItem, i) => (
                    <List.Item
                      ...
                      actions={[
                        ...
                        <SwitchLink type="link" key="list-view">
                          view
                        </SwitchLink>,
                      ]}
                    >
                      <List.Item.Meta
                        ...
                        title={
                          <StyledSpanHeading>
                            {`${messageItem.title} (${
                              messageItem.replies.length
                            } ${
                              messageItem.replies.length === 1
                                ? "reply"
                                : "replies"
                            })`}
                          </StyledSpanHeading>
                        ...
                        } ${getTimeFrameFromNow(messageItem.Created_date!)}`}
                      />
                    </List.Item>
                  )}
                />
          ...
          <Modal
            ...
            onOk={modalAction}
            okText={
              actionState === MessageActionState.Deleting ? "Delete" : "Flag"
            }
            onCancel={() => {
              setInProgress(false);
            }}
          ...
          >
            ...
            <Paragraph>
              Good to have you aboard. Here is some useful info to get you up
              and running.
            </Paragraph>
            <Timeline>
              <Timeline.Item>Fill in your profile</Timeline.Item>
              <Timeline.Item>
                Send some updates about your service to others
              </Timeline.Item>
